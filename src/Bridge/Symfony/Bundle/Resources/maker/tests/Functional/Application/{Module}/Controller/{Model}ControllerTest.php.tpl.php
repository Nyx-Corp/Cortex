...
namespace Tests\Functional\Application\<?php echo $Module; ?>\Controller;

use Tests\Functional\Application\<?php echo $Module; ?>\<?php echo $Module; ?>WebTestCase;

class <?php echo $Model; ?>ControllerTest extends <?php echo $Module; ?>WebTestCase
{
    /**
    ...
    public function testIndexReturns200(): void
    {
        ...
        $client->request('GET', '/<?php echo $model; ?>s');
    ...
    {
        $client = static::createAppClient();
        $client->request('GET', '/<?php echo $model; ?>/create');
    ...
    {
        $client = static::createAppClient();
        $crawler = $client->request('GET', '/<?php echo $model; ?>/create');

        $form = $crawler->selectButton('submit-btn')->form();
        // TODO: Fill required fields
        // $form['<?php echo $model; ?>_edit[field]'] = 'value';
        $client->submit($form);

        $this->assertResponseRedirects();
        $this->assertMatchesRegularExpression('#/<?php echo $model; ?>/[a-f0-9-]{36}/edit#', $client->getResponse()->headers->get('Location'));
    }
    ...
    public function testCreatedItemAppearsInList(): void
    {
        ...
        $crawler = $client->request('GET', '/<?php echo $model; ?>s');
        $this->assertResponseIsSuccessful();
    ...
    public function testEditPageLoadsWithExistingItem(): void
    {
        ...
        $crawler = $client->request('GET', sprintf('/<?php echo $model; ?>/%s/edit', $uuid));
        $this->assertResponseIsSuccessful();
        $this->assertSelectorExists('form');
    }
    ...
    public function testFilterByUuidDisplaysActiveFilterChip(): void
    {
        ...
        $uuid = $this->createTestItem($client);
        $crawler = $client->request('GET', sprintf('/<?php echo $model; ?>s?q=uuid:%s', $uuid));
    ...
    public function testEditFromListAndVerifyChanges(): void
    {
        ...
        $this->assertResponseIsSuccessful();
        $labelField = $crawler->filter('#submit-btn')->form()->get('<?php echo $model; ?>_edit[' . self::LABEL_FIELD . ']');
        $this->assertEquals($initialLabel, $labelField->getValue());

        // 4. Modify the value
        $modifiedLabel = 'Modified-' . uniqid();
        $form = $crawler->filter('#submit-btn')->form();
        $form['<?php echo $model; ?>_edit[' . self::LABEL_FIELD . ']'] = $modifiedLabel;
        $client->submit($form);
        ...
        $listLink = $crawler->filter('a[href$="/<?php echo $model; ?>s"]');
        ...
        $crawler = $client->request('GET', sprintf('/<?php echo $model; ?>s?q=%s:%s', self::LABEL_FILTER, $modifiedLabel));
        $this->assertResponseIsSuccessful();
    ...
    //
    //     // Archive
    //     $client->request('GET', sprintf('/<?php echo $model; ?>/%s/archive', $uuid));
    //     $this->assertResponseRedirects();
    //
    ...
    //     $crawler = $client->request('GET', '/<?php echo $model; ?>s?q=archivedAt:true');
    //     $this->assertResponseIsSuccessful();
    //     $this->assertSelectorExists(sprintf('input[value="%s"]', $uuid));
    // }
    ...
    private function createTestItem($client, ?string $label = null): string
    {
        ...
        $form['<?php echo $model; ?>_edit[' . self::LABEL_FIELD . ']'] = $label;
        $client->submit($form);
